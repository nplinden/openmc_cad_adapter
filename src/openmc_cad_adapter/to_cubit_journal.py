...
def to_cubit_journal(
    ...
    if to_cubit:
        try:
            ...
            raise ImportError(
                "Cubit Python API not found. Please install Cubit to use this feature."
            )
    ...
    if world is None:
        ...
        if not all(np.isfinite(bbox[0])) or not all(np.isfinite(bbox[1])):
            raise RuntimeError(
                "Model bounds were not provided and the bounding box determined by OpenMC is not finite."
    ...
    if world is None:
        raise RuntimeError(
            "Model extents could not be determined automatically and must be provided manually"
        )
    ...
    def surface_to_cubit_journal(
        ...
        if isinstance(node, Halfspace):
            ...
            if cad_surface := _CAD_SURFACE_DICTIONARY.get(surface._type):
                ...
                ids, cad_cmds = cad_surface.to_cubit_surface(
                    ent_type, node, w, inner_world, hex
                )
                cmds += cad_cmds
                return ids
                ...
                def rotation_to_axis_angle(mat):
                    ...
                    if abs(theta) <= np.finfo(np.float64).eps:
                        ...
                        col = 0
                        if mat[1, 1] > mat[col, col]:
                            col = 1
                        if mat[2, 2] > mat[col, col]:
                            col = 2
                ...
                if gq_type == gqs.ELLIPSOID:  # 1
                    ...
                    if A_ == 0:
                        ...
                        r2 = math.sqrt(abs(K_ / B_))
                        cmds.append(
                            f"cylinder height {h} Major Radius {r1} Minor Radius {r2}"
                        )
                        ids = emit_get_last_id(ent_type, cmds)
                        ...
                        if node.side != "-":
                            ...
                            if inner_world:
                                if hex:
                                    ...
                                    cmds.append(
                                        f"rotate body {{ {wid} }} about z angle 30"
                                    )
                                    cmds.append(
                                        f"rotate body {{ {wid} }} about y angle 90"
                                    )
                                else:
                                    cmds.append(
                                ...
                                wid = emit_get_last_id(ent_type, cmds)
                            cmds.append(
                                f"subtract body {{ { ids } }} from body {{ { wid } }}"
                            )
                            cmds.append(
                                f"Rotate body {{ {wid } }} about 0 0 0 direction {r_axis[0]} {r_axis[1]} {r_axis[2]} Angle {r_degs}"
                            )
                            move(
                                wid,
                                translation[0, 0],
                            ...
                            return wid
                        cmds.append(
                            f"Rotate body {{ {ids} }} about 0 0 0 direction {r_axis[0]} {r_axis[1]} {r_axis[2]} Angle {r_degs}"
                        )
                        move(
                            ids,
                            translation[0, 0],
                    ...
                    if A_ == 0:
                        ...
                        cmds.append(
                            f"rotate body {{ { mirror } }} about 0 0 0 angle 180"
                        )
            ...
            id = surface_to_cubit_journal(
                node.node, w, indent + 1, inner_world, ent_type=ent_type
            )
            cmds.append(f"brick x {w[0]} y {w[1]} z {w[2]}")
            wid = emit_get_last_id(ent_type, cmds)
            ...
            if len(node) > 0:
                local_ent_type = "body"
                first = surface_to_cubit_journal(
                    node[0], w, indent + 1, inner_world, ent_type=local_ent_type
                )
                for subnode in node[1:]:
                    s = surface_to_cubit_journal(
                        subnode, w, indent + 1, inner_world, ent_type=local_ent_type
                    )
                    ...
                    for uss in node.universes:
                        ...
                        for us in uss:
                            ...
                            def draw_hex_cell(n, cell, x, y):
                                ...
                                id = process_node_or_fill(
                                    cell,
                                    [w[0], w[1], w[2]],
                                    indent + 1,
                                    offset=[x, y, z],
                                    inner_world=[pitch[0], pitch[0], pitch[1]],
                                    outer_ll=outer_ll
                                    if outer_ll
                                    else [node.center[0], node.center[1]],
                                    hex=True,
                                )
                                ids = str(id)
                                ...
                                if ids != "":
                                    cmds.append(
                                        f"move body {{ {ids} }} midpoint location {x} {y} {z}"
                                    )

                            side_to_side_diameter = pitch[0] / 2 * math.sqrt(3)
                            center_to_mid_side_diameter = (
                                (pitch[0] / 2) * math.sin(math.pi / 6)
                            ) + pitch[0] / 2
                            if ring_id < 2:
                                for u in us:
                                ...
                                for i in range(r, 0, -1):
                                    ...
                                    y_pos = (
                                        ring_id * side_to_side_diameter
                                        - (x) * 0.5 * side_to_side_diameter
                                    )
                    ...
                    for u in us:
                        for n, cell in u._cells.items():
                            ...
                            id = process_node_or_fill(
                                ...
                                indent + 1,
                                offset=[x, y],
                                inner_world=[pitch[0], pitch[1], w[2]],
                                outer_ll=outer_ll
                                if outer_ll
                                else [node.lower_left[0], node.lower_left[1]],
                            )
                            ids = str(id)
                            if isinstance(id, list):
                                ids = " ".join(map(str, id))
                            ...
                            if ids != "":
                                cmds.append(
                                    f"move body {{ {ids} }} midpoint location {x} {y} 0 except z"
                                )
                        j = j + 1
                    i = i + 1
    ...
    def do_cell(cell, cell_ids: Iterable[int] = None):
        ...
        if cell.fill is None:
            ...
            if len(mat_identifier) > 32:
                ...
                warnings.warn(
                    f"Truncating material name {mat_identifier} to 32 characters"
                )
...
def material_assignment(cell, geom_id, assignment_type="group"):
    ...
    if assignment_type == "group":
        ...
        raise ValueError(
            f"Unknown material assignment type requested: {assignment_type}"
        )
...
def openmc_to_cad():
    ...
    parser.add_argument(
        "input",
        help="Path to a OpenMC model.xml file or path to a directory containing OpenMC XMLs",
    )
    parser.add_argument("-o", "--output", help="Output filename", default="openmc.jou")
    parser.add_argument(
        "-w",
        "--world-size",
        help="Maximum width of the geometry in X, Y, and Z",
        nargs=3,
        type=int,
    )
    parser.add_argument(
        "-c", "--cells", help="List of cell IDs to convert", nargs="+", type=int
    )
    parser.add_argument(
        "--to-cubit", help="Run  Cubit", default=False, action="store_true"
    )
    parser.add_argument(
        "--cubit-path", help="Path to Cubit bin directory", default=None, type=str
    )
    ...
    if model_path.is_dir():
        ...
        model = openmc.Model.from_xml(
            *[model_path / f for f in ("geometry.xml", "materials.xml", "settings.xml")]
        )
    else:
